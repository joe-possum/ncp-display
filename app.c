...
{
  if(!booted) return;
  printf("app_process_action()\nrow: %d\n",row);
  sl_status_t rc;
  if(row < 128) {
    uint8_t buf[256];
    size_t len;
    buf[0] = row;
    uint8_t row_count = 128 - row;
    if(row_count > 15) row_count = 15;
    buf[1] = row_count;
    printf("before memcpy()\n");
    memcpy(buf+2,&screen[16*row],row_count*16);
    printf("after memcpy()\n");
    rc = sl_bt_user_message_to_target(row_count*16+2,buf,255,&len,buf);
    printf("rc: %d, len: %ld\n",rc, len);
